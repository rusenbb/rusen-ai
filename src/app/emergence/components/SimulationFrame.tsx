...
export function useSimulation() {
  ...
  const random = useMemo(
    () => createSeededRandom(context.seed),
    [context.seed],
  );
...
export default function SimulationFrame({ children }: { children: ReactNode }) {
  ...
  return (
    <div ref={root} className="min-h-80">
      <div className="mb-5 flex flex-wrap items-center gap-2 text-xs">
        ...
        <span className="text-neutral-500">
          Replay restores this seed and default controls. Offscreen simulations
          pause.
        </span>
      </div>
      {started ? (
        <SimulationContext.Provider
          key={`${seed}-${replay}`}
          value={{ seed, active: visible && foreground }}
        >
          {children}
        </SimulationContext.Provider>
      ) : (
        <p className="text-sm text-neutral-500">
          Simulation initializes when this section enters the viewport.
        </p>
      )}
    </div>
  );
